...
Funcion return<- menu() //funcion para validar las opciones del menu
	...
	Repetir
		...
		Escribir "2--> LOGGIN"
		Escribir "3--> CONSULTE DESTINOS Y RESERVE VUELOS"//En esta versión los orígenes/destinos se muestran como un listado con cada provincia con un nro asignado y no hay búsqueda, se muestra tmb l zona traifaria que vamos a usar para calcular el costo del pasaje
		Escribir "4--> VER EL ESTADO DE LA RESERVA" //el usuario puede buscar su reserva por nro de reserva.
		Escribir "5--> ADMINISTRADOR (ACCESO RESTRINGIDO)" // este usuario va a tener un loggin y pass hardcodeado, respresenta a un empleado de la empresa que va a poder realizar distintas búsquedas/ordenamientos
		Escribir "6--> SALIR"
		Leer opcionM
		Limpiar Pantalla
...
SubProceso  registrarUsuario(usuarios Por Referencia, cantidadUsuarios Por Referencia,numeroDeUsuario Por Referencia)
	...
	Para i<-0 Hasta cantidadUsuarios-1 Hacer 
		Escribir "NOMBRE:  -> ",usuarios[i,0]
		Escribir "APELLIDO:  -> ",usuarios[i,1]
		Escribir "DNI:  -> ",usuarios[i,2]
		Escribir "EDAD: -> ",usuarios[i,3]
		ESCRIBIR "CONTRASEÑA:  -> ",usuarios[i,4]
		Escribir "NRO DE USUARIO: -> ",usuarios[i,5]
	...
	//----------------------------//
		...
		Repetir
				...
				SiNo
					...
					mostrar "4 - Salir"  //mostrar arreglo
...
//NO USAR ------ BORRAR DE LA VERSION DEFINITIVA, NO FUNCIONA ------
//SubProceso mostrarVentasPorUsuario(arregloUsuarios, n, m) //, arregloReservas, arregloEquipaje) muestra las ventas realizadas, vistas por usuario registrado
//	Definir fila, columna Como Entero
//	Definir contarFilaUsuario, contarColumnaUsuario como entero
//	contarFilaUsuario<-0
//	contarColumnaUsuario<-0
//	
//	Para i<-0 hasta n-1 hacer
//		contarFilaUsuario<-contarFilaUsuario + i
//		para j<-0 hasta m-1 Hacer
//			contarColumnaUsuario<-contarColumnaUsuario + j
//			si arregloUsuarios[i,j] <> "" Entonces
//				fila<-i
//				columna<-j
//				//Escribir "    NOMBRE    -    APELLIDO    -    DNI    -    EDAD    "
//				Escribir Sin Saltar arregloUsuarios[fila,columna], " "
//				//mostrar "----> Reserva del usuario <---- "
				//mostrar "Origen    -  Destino   -   Horario    -    Cod. Reserva   -  Costo Pasaje"
				//Mostrar arregloReservas[fila,columna]
				//mostrar "Tipo de equipaje    -  Cantidad   -   Costo Equipaje"
				//mostrar arregloEquipaje[fila,columna]
//			fin si
			//		FinPara
		//		Escribir ""
		//	finpara
	
	//Mostrar contarFilaUsuario+1
	//Mostrar contarColumnaUsuario+1
	
	//FinSubProceso



SubProceso buscarUsuarioRegistrado(arreglo, n, m, buscar) //busca usuario registrado por apellido
	Definir fila, columna Como Entero 
